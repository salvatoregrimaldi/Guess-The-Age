...
data = pd.read_csv(csv_name, nrows=num_samples, header=None) # data is a DataFrame
array_temp = data.to_numpy() # converting DataFrame into numpy array
...
#Get train and validation lenght
train_len = int(num_samples*DATASET_SPLIT[0])
validation_len = int(num_samples*DATASET_SPLIT[1])
...
for fold in range(5):
  ...
  train_idxs = np.concatenate((idxs[:(fold)*validation_len],idxs[(fold+1)*validation_len:]))
  validation_idxs = idxs[(fold)*validation_len:(fold+1)*validation_len]

  print("train_idxs is ixds[ :",(fold)*validation_len, "] and [", (fold+1)*validation_len, ": ]")
  print("validation_idxs is ixds[",(fold)*validation_len, ":",(fold+1)*validation_len, "]")
  ...
  # Testing new representation stage

  model_name = 'A3MoreDenseFold' + str(fold)

  EPOCHS = 100

  print("REPRESENTATION STAGE")

  model, age_model, _ = build_model("resnet50")
  ...
  h_repr = model.fit(train_sequence, 
          workers = 32,
          max_queue_size = 10,
          ...
          callbacks=[
              ComputeAARCallback(),
              MonitorValAAR('/user/2022_va_gr07/AVProject/model_saves/' +model_name +'_repr' +'.h5', patience=4)
              ],
          verbose=1,
          )
